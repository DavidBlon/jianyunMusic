...
当前版本：**2.4.4**（Android 8.0 及以上）

## 2.4.4 更新

- 界面整体调整为苹果风格设计：iOS 系统级配色、磨砂玻璃卡片材质与字距分级排版。
- 新增「跟随系统 / 浅色 / 深色」三种主题模式，以及 6 套 iOS 系统强调色（浅色模式自动使用对应浅色变体）。
- 修复自定义背景（图片/视频）下玻璃卡片填充渐变导致的"底部渐变黑"观感。
- 修复浅色模式下播放页与迷你播放器的播放/暂停键对比度、底部导航栏文字截断问题。
