...
function persist() {
  const { route, results, loading, panel, playing, key, ...saved } = state;
...
function normalizeWy(item) {
  if (item.title && item.url) {
    ...
    return { id, remoteId: id, songmid: id, source: "linglan.wy", sourceName: "网易云", title: item.title || "未知歌曲", artist: item.author || "未知歌手", album: item.album || "", duration: Number(item.duration || 0), artwork: secureUrl(item.pic) };
  }
...
async function loadRecommendedPlaylists() {
  ...
  if (state.sourceId === "linglan.wy") {
    const body = await fetchJson("https://music.163.com/api/playlist/list?cat=%E5%85%A8%E9%83%A8&order=hot&offset=0&total=true&limit=18");
    return (body?.playlists || []).map((item) => normalizePlaylist(item));
  }
  return [];
}
...
async function validateSourceKey(key) {
  ...
  const endpoint = new URL(`${SOURCE_API}/api/script`);
  endpoint.searchParams.set("checkUpdate", "jiany-music-web-static");
  endpoint.searchParams.set("key", key.trim());
  const body = await fetchJson(endpoint);
  const returnedKey = String(body.currentkey || "").trim();
  if (Number(body.code) !== 200 || !returnedKey || returnedKey !== key.trim()) throw new Error("卡密无效或已失效");
  return true;
}
...
function miniPlayer() {
  ...
  return `<div class="mini-player"><img src="${art(state.current.artwork)}" alt="" onerror="this.src='${FALLBACK_ART}'"><div class="mini-copy" data-action="route" data-route="player"><strong>${esc(state.current.title)}</strong><span>${esc(state.current.artist)}</span></div><button class="icon-button" data-action="play-toggle" aria-label="${state.playing ? "暂停" : "播放"}">${icon(state.playing ? "pause" : "play")}</button><button class="icon-button" data-action="next" aria-label="下一首">${icon("next")}</button></div>`;
}
...
function renderPlaylist() {
  ...
  return shell(`<main class="screen"><header class="playlist-hero" style="background-image:url('${art(heroArt)}')"><div class="hero-actions"><button class="icon-button" data-action="back">${icon("back")}</button><button class="icon-button" data-action="playlist-search">${icon("search")}</button></div><div><div class="playlist-type">${esc(list.type || "歌单")}</div><h1 class="playlist-name">${esc(list.name)}</h1><div class="playlist-count">${list.tracks.length} 首歌</div></div></header><button class="primary-button play-all" data-action="play-all" data-id="${list.id}">${icon("play", "fill")} 播放全部</button><div class="track-list">${list.tracks.map((track, index) => resultRow(track, index, `playlist:${list.id}`)).join("")}${list.tracks.length ? "" : '<div class="loading dim">暂无歌曲</div>'}</div></main>`);
}
...
async function loadDiscover() {
  ...
  try {
    await ensureConnected();
    const [playlists, tracks] = await Promise.allSettled([loadRecommendedPlaylists(), searchSource("热门歌曲")]);
    discoveryPlaylists = playlists.status === "fulfilled" ? playlists.value : [];
    discoveryTracks = tracks.status === "fulfilled" ? tracks.value.slice(0, 18) : [];
    if (!discoveryPlaylists.length && discoveryTracks.length) {
      discoveryPlaylists = ["热门歌曲", "经典回响", "今日聆听"].map((name, index) => ({ id: `smart-${index}`, name, artwork: discoveryTracks[index * 3]?.artwork, source: state.sourceId, tracks: discoveryTracks.slice(index * 6, index * 6 + 9) }));
    }
...
async function resolveAndPlay(track) {
  if (track.url) { $audio.src = track.url; return $audio.play(); }
  await ensureConnected();
  ...
  const body = await fetchJson(endpoint, { headers: { "X-API-Key": state.key, "Content-Type": "application/json" } });
...
async function playTrack(track, list) {
  ...
  render();
  try { await resolveAndPlay(track); } catch (error) { state.playing = false; render(); toast(error.message); }
  updateMediaSession();
}
...
$app.addEventListener("click", async (event) => {
  ...
  if (action === "route") { if (target.dataset.route === "player" && state.route !== "player") state.returnRoute = state.route; state.route = target.dataset.route; state.panel = null; render(); return; }
  if (action === "back") { state.panel = null; state.route = state.route === "player" ? (state.returnRoute || "discover") : "my"; render(); return; }
  ...
  if (action === "open-playlist") { state.playlistId = target.dataset.id; state.route = "playlist"; render(); return; }
  if (action === "open-discovery-playlist") { const selected = discoveryPlaylists.find((item) => String(item.id) === String(target.dataset.id)); if (!selected) return; target.style.pointerEvents = "none"; toast("正在加载歌单…"); try { const tracks = selected.tracks?.length ? selected.tracks : await loadPlaylistTracks(selected); state.playlistId = "discover"; state.playlists = state.playlists.filter((list) => list.id !== "discover"); state.playlists.push({ id: "discover", name: selected.name || "推荐歌单", type: sourceMeta(selected.source).name, tracks: tracks.length ? tracks : discoveryTracks }); state.route = "playlist"; render(); } catch (error) { if (discoveryTracks.length) { state.playlistId = "discover"; state.playlists = state.playlists.filter((list) => list.id !== "discover"); state.playlists.push({ id: "discover", name: selected.name || "推荐歌单", type: sourceMeta(selected.source).name, tracks: discoveryTracks }); state.route = "playlist"; render(); } else { target.style.pointerEvents = ""; toast(error.message || "歌单加载失败"); } } return; }
  if (action === "playlist-search") { state.route = "search"; render(); return; }
...
$app.addEventListener("pointerdown", (event) => {
  ...
  dragScroll = { scroller, startX: event.clientX, scrollLeft: scroller.scrollLeft, moved: false };
...
$app.addEventListener("pointermove", (event) => {
  ...
  dragScroll.moved ||= Math.abs(delta) > 6;
  dragScroll.scroller.scrollLeft = dragScroll.scrollLeft - delta;
});
$app.addEventListener("pointerup", () => {
  if (!dragScroll) return;
  const { scroller, moved } = dragScroll;
  scroller.classList.remove("dragging");
  if (moved) scroller.dataset.dragged = "1";
